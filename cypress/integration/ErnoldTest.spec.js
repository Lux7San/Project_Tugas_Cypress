...
describe('From input', () => {
    ...
    it('Fill Inline Forms',() => {
        cy.get('input[placeholder="Jane Doe"]').type('Ernold Mochammad Can Setya')
        cy.get('[placeholder ="Email"][type="text"]').type('ernolds.es@gmail.com')
        cy.get('.custom-checkbox').first().click()
    });

    it('Using Grid',()=> {
        cy.get('#inputEmail1').type('ernolds.es@gmail.com')
    ...
    it('Basic Form',()=> {
        cy.get('#exampleInputEmail1').type('ernolds.es@gmail.com')
        cy.get('#exampleInputPassword1').type('123456789')
        cy.get('.text').contains('Check me out').click()
    })

    it('Form Without Labels',()=>{
        cy.get('[placeholder="Recipients"]').type('Ernold Mochammad Can Setya')
        cy.get('[placeholder="Subject"]').type('Tugas PPL BlackBoxTesting')
        cy.get('[placeholder="Message"').type('Selamat Mengerjakan')
    })

    it('Block Form',()=>{
        cy.get('#inputFirstName').type('Ernold')
        cy.get('#inputLastName').type('Mochammad Can Setya')
        cy.get('#inputEmail').type('ernolds.es@gmail.com')
        cy.get('#inputWebsite').type('www.wibusubs.com')
    })

    it('Horizontal Form',()=> {
        cy.get('#inputEmail3').type('ernolds.es@gmail.com')
